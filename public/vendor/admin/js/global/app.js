      ...
      switch (action) {
        ...
        case "card-collapsed":
          ...
          if ($card.parents('.card').length === 0) {
            $card.find(".card-body:not(.card-body .card-body)").slideToggle();
          } else {
...
(function(window, document, $, undefined) {
  ...
  $(function() {
    ...
    // -- Auto hidden flash messages
    $('#flash-overlay-modal').modal();


    // -- CodeMirror
    var codeEditor = document.querySelectorAll('.cnv-code-editor');

    if (codeEditor.length > 0) {
          for (let i = 0; i < codeEditor.length; i++) {
              CodeMirror.fromTextArea(codeEditor[i], {
                  lineNumbers: true,
                  mode: "htmlmixed"
              });
          }
